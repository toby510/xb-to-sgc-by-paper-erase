...
根对象固定为：`page_id`、`decision`、`original_target_is_non_body`、`body_changed`、`target_removed`、`background_acceptable`、`evidence`。

- `original_target_is_non_body`、`body_changed`、`target_removed` 是独立布尔判断；`body_changed=true` 表示正文被改变（伤正文）。
- `decision=pass` 当且仅当 `original_target_is_non_body=true`、`body_changed=false`、`target_removed=true`；`background_acceptable` 仅表示视觉质量告警。
