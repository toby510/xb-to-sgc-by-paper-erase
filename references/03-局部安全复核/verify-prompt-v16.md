...
返回 `safe_to_erase` 前，必须复看你给出的 `refined_region`：该框内必须实际包含所匹配页码或同行元数据的可见字符笔画，四边应贴近最外笔画并仅留极小抗锯齿余量。不得只因“页码应在底部/顶部”而把框落在文字下方或旁边的空白区；若无法同时量到完整字形和安全边界，返回 `manual_review`。

按以下顺序做最后一次测量：先找目标行首字符的最左笔画和末字符的最右笔画，作为 `x1/x2`；再找整行最高与最低笔画，作为 `y1/y2`。逐字复核 `page_number_text` 中每个实际可见字符均在框内；只框中间数字、只框部分 `第/页/共`、或让任一首末字符落在框外，都不得返回 `safe_to_erase`。
