...
判定一行文字属于正文还是页眉/页脚元数据，只以它在页面中的**位置证据**为准，不得以“文字内容看起来像什么”为准：

1. 当一行文字位于页面物理边缘的终止带内（上方候选到页面上边缘、下方候选到页面下边缘、左右候选到对应侧边之间有大量连续空白或页边空白），且它与最近正文块之间存在连续可见空白带时，该行属于页眉/页脚元数据。
2. 上述口径同样适用于文字内容形如章节名、单元名、课文名、专题名、学校名或试卷名的运行标题/页眉页脚文字。它们出现在页面终止带内，就是不参与正文阅读流的元数据，不得因为“看起来像正文标题或正文结构元素”而改判为正文。
3. 页眉/页脚分隔线（横跨页面、位于页边带内、与该行文字相邻的细线）是元数据带的一部分及其与正文块的分界，不构成“该行属于正文”的证据。文字在该分界线的**页面边缘一侧**时仍属于元数据；只有文字在分界线**正文一侧**、并与正文行距连续时才属于正文。
4. 只有同时满足“不处于页面终止带内”或“与正文之间没有连续空白带/与正文连通”时，才可判定为正文并返回 `manual_review`。判定前必须先用画面确认页面物理边缘与正文块的位置，再做结论。
