...
3. `target_removed`：逐个 `region_id` 判断后取 AND。页码和获批同行非正文元数据都必须消失；仍可识别任何获批文字、`第/页/共` 或数字字形时必须为 false。不可识别的浅灰背透、纸纹、抗锯齿、孤点和纯色残影不算目标残留。
...
{"page_id":"REQUEST_PAGE_ID","decision":"pass","original_target_is_non_body":true,"body_unchanged":true,"target_removed":true,"background_acceptable":false,"evidence":"ORIGINAL 中目标为独立页脚页码；ERASED 中页码已不可识别，整页正文未变；仅有轻微背景色差"}
