...
## 主判据：看"被涂掉的那块区域"原来有什么

不要去数框边缺口有几个像素——那个缺口太小，容易看漏。改用对齐判断，按下面三步做：

1. 在 ERASED 局部图里找出获批框对应的**涂白/平涂色块**（一块与周围纸张背景明显不同的实心区域）。
2. 把这块色块的位置**原样对应到 ORIGINAL 局部图的同一位置**，看那块区域在 ORIGINAL 里原本是什么。
3. 按下面两条判定：
   - ORIGINAL 中该色块覆盖范围内**只有页码、同行元数据、纯背景或纸纹** → 正文没被动过，`body_changed=false`。
   - ORIGINAL 中该色块覆盖范围内**出现任何正文文字的笔画**（哪怕只有字的下缘、收笔、横画或标点的一小部分）→ 这些笔画已经被涂掉，属于伤害，`body_changed=true`。

一句话口径：**被涂掉的那块区域在 ORIGINAL 里含正文笔画，就是伤正文**，不需要判断切口有多大、也不需要判断缺口是否"明显"。

这条主判据比逐边找缺口更可靠，因为它只需要比较"同一块区域在两图里内容是否一致"，不依赖分辨 1-5 像素的细节。逐边检查仍然要做，但只用来补充确认。
