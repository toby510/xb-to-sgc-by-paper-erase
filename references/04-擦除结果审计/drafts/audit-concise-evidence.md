...
- **`evidence` 必须简短：整段不超过 80 个汉字。** 逐边结论用极短格式，例如「上：危险（切口）；下：安全；左：安全；右：安全」。不要复述规则、不要展开推理、不要解释判断过程；`evidence` 写太长会导致响应被截断、整条审计作废。
- **七个字段一个都不能少**，输出顺序固定为：`page_id`、`decision`、`original_target_is_non_body`、`body_changed`、`target_removed`、`background_acceptable`、`evidence`。先写完前面六个字段，再写最短的 `evidence`。
