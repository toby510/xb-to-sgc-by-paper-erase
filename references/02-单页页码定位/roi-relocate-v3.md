...
5. 紧邻目标行的通栏分隔线、下划线等长横线必须完整排除在 refined_region 之外：朝向横线的那条边必须收在"目标文字笔画带与横线之间的空白带"内，横线的任何一个像素都不得落入框内。量化切法：refined_region 朝向横线的边 = 目标文字最外笔画位置向外仅扩约 1-2 个像素（归一化坐标约 0.001-0.002），到此为止，不得继续向横线方向延伸；横线与框边之间的空白全部留在框外。宁可框边紧贴目标文字笔画，也绝不允许把横线任何一行像素包进框内。
...
4. refined_region 上下边必须超出目标行最高笔画（含全角括号、长撇捺），使最高单笔画高度明显小于框高：框高至少约为目标文字笔画高度的 1.5 倍，防止单根竖向笔画纵贯整个框高。
5. 候选行同视觉带上但与目标无文字连通、且被连续空白分开的图形、表格或正文，不得仅因同高就视为目标的一部分，也不得仅因此否定目标。
6. refined_region 朝正文方向的边不得切穿正文笔画：先找到最近正文最外笔画位置 C 和目标最外笔画位置 D，该侧边必须同时满足：①与 C 的距离 ≥ 0.005（约 8 像素，下游门禁硬要求，小于此值一律被拦截）；②不切目标笔画且与 D 之间保留约 0.001-0.002 空白。满足条件的取值 = C + 0.005 到 C + 0.006 之间（贴近下限，把更多空白留给目标侧），坐标保留三位小数。若 ROI 中 C 与 D 之间不存在同时满足①②的位置（空白带不足约 0.007），返回 `target_found:false` 而不是硬挤坐标。
7. **最终坐标必须根据当前 ROI 中重新看到的目标测量，不得继承旧框边界。**
...
3. `refined_region` 是未切笔画、不吞入其他内容的最小完整框，且已把紧邻的分隔线、下划线等长横线完整排除在框外，框高约为目标文字笔画高度的 1.5 倍以上。
