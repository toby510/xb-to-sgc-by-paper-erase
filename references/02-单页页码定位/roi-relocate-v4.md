...
6. refined_region 朝正文方向的边不得切穿正文笔画：该边落在目标与最近正文之间的空白带里、贴近目标一侧，并与最近的正文笔画保持肉眼可见的空白；留白宽度不必精确，能看清两边都没有粘连即可。若 ROI 中空白带过窄、无法同时做到不切目标笔画、不切正文笔画且两边都留可见空白，返回 `target_found:false` 而不是硬挤坐标。
