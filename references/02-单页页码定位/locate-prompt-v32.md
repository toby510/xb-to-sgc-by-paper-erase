...
当前输入尚未旋正且 `reading_rotation != 0` 时，严禁返回擦除坐标，必须 `status:"manual_review"`、`regions:[]`；Java 会旋正原图后重新定位。已旋正整页或局部 ROI 的 `reading_rotation` 必须为 `0`。
...
3. region 必须完整包围该独立目标行全部可见页码字符及连续同行非正文元数据；四边不得切穿目标笔画，并在四周保留少量清晰可见空白。朝正文方向的边必须位于目标与正文之间的连续空白带内，不得越过空白带触及正文。长页脚行必须完整覆盖上下笔画，且不得跨过空白带吞入其他内容。
...
8. 页码与同一连续独立页脚徽标、色块或闭合装饰外框构成一个元数据带时，region 必须完整包围该带全部可见目标字符与连续同行非正文元数据；四边不得切穿目标笔画，并在四周保留少量清晰可见空白；朝正文方向的边必须位于目标与正文之间的连续空白带内，不得越过空白带触及正文。不得只框内部数字，也不能跨越空白去扩框吞入其他内容；该带仍可 `on_line=false`。

## 输出前自检

- 框内是否覆盖全部页码与同基线明确非正文元数据，含最右/最左字符？
- 框是否完整包围目标行全部可见字符与连续同行非正文元数据，四边未切穿目标笔画且四周留有少量清晰可见空白？朝正文方向的边是否位于目标与正文之间的连续空白带内、未跨过空白带触及正文？否则返回 `manual_review`。
...
- **层级自检**：方向字段、`nearest_body_boundary` 与 `evidence` 必须且只能出现在 JSON 根层级；`regions` 数组内每个 region 对象只允许包含协议列出的十个字段，其中不得出现根层级字段。
...
3. `nearest_body_boundary` 与 `evidence` 是**根层级字段**：任何情况下都不得写进 `regions` 数组的 region 对象内，也不得省略。
4. `regions` 数组内每个 region 对象**只允许**这十个字段：`region_id`、`x1`、`y1`、`x2`、`y2`、`page_number_text`、`same_line_metadata`、`on_line`、`confidence`、`safety_margin`；不得增加、删除或混入根层级字段。
5. 即使返回多个 region，也只写**一份**根层级的 `nearest_body_boundary` 与 `evidence`，汇总描述全部 region，不得按 region 分别嵌套。
6. `nearest_body_boundary` 必须有 `x`、`y`、`basis` 三字段：底部/顶部目标填 `y` 且 `x:null`；左侧/右侧目标填 `x` 且 `y:null`；两轴字段名必须都写出，无值时显式 `null`，不得省略。`basis` 非空，只能说明清晰前景正文而非背透/浅影；当前输入内没有清晰正文时两个坐标都为 null。
7. `safe_to_erase` 时每个 region 的 `page_number_text` 必须是当前图肉眼可见、非空的精确页码字面量，`regions` 非空；`no_pagenum` 与 `manual_review` 时 `regions` 必须为空。`region_id` 从 `r1` 按当前阅读顺序编号；`confidence` 是当前目标和坐标均正确的把握；所有枚举仅使用示例列出的值。
