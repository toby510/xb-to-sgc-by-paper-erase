...
提示词改动一律按下面的流程走，保证任何一次运行的结论都能追到"哪一版、改了什么、为什么"：

1. **新建版本文件，不动历史版本**：`locate`、`roi-relocate`、`audit` 各自独立编号，文件名带版本号。
2. **同步登记 `references/提示词版本索引.md`**：新增一行，写全"变更 / 为什么改 / 验证 / 状态"，并把旧版状态改成"历史"。
3. **改 `config/vlm-providers.json` 指向新版本**，确保同一时间每个角色只有一个"当前"版本。
4. **先在草稿里试，验证通过再冻结版本号**：还没跑通的调整不要占用版本号，避免版本号膨胀。
5. **每次运行自动留痕**：`run.json` 记录提示词路径与 sha256，快照存在 `runs/<run>/metadata/prompts/`，可用于复盘某次结果到底用了哪一版。

可选自检：`bash scripts/check-prompt-index.sh`，检查 config 指向的版本是否都已在索引中登记为"当前"。
